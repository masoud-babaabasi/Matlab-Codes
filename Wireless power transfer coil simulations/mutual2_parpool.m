...
%       a = (j-1) + (i-1)*N1 + 1;
      P1 = [ x ; Range1(j) ; cz];
      if( sqrt((x - cx)^2 + (Range1(j) - cy)^2) < (R1 - w/2) ) 
        B1(:,i,j) = integral(@(theta)intgral_func(theta,R1,C1,P1,I,eye(3)),0,2*pi,'ArrayValued',true);
      end
   end
end
L1 = abs(sum(sum(B1(3,:)))) * dx1 * dx1 / I;
...
%       a = (j-1) + (i-1)*N2 + 1;
      P2 = [ x ; Range2(j) ; cz];
      if( sqrt((x - cx)^2 + (Range2(j) - cy)^2) < (R2 - w/2) )
        B2(:,i,j) = integral(@(theta)intgral_func(theta,R2,C2,P2,I,eye(3)),0,2*pi,'ArrayValued',true);
      end
   end
end
L2 = abs(sum(sum(B2(3,:)))) * dx2 * dx2 / I;
%%
D = -4*R1:5e-3:4*R1;
...
%                   P_r = eye(3) * P1(:,a);
                    P1 = [ x ; Range1(j) ; cz];
                    P_r = eye(3) * P1;
                  if( sqrt((P_r(1) - cx)^2 + (P_r(2) - cy)^2) < (R1 - w/2) ) && ( (sqrt((P_r(1) - cx2)^2 + (P_r(2) - cy2)^2) > (R2 + w/2) || sqrt((P_r(1) - cx2)^2 + (P_r(2) - cy2)^2) < (R2 - w/2) || P_r(3) < (cz2-w/2) || P_r(3) > (cz2+w/2) ))
                    B21(:,i,j) = integral(@(theta)intgral_func(theta,R2,C2,P1,I,r),0,2*pi,'ArrayValued',true);
        ...
        %             end
           ...
           %B21 = r*B21;
          MM(di) = sum(sum(B21(3,:))) * dx1 * dx1 / I;
        %     disp(phi12(1,find(D==d)))
        end
        M21((ii-1)*N_tilt+jj,:) = MM;
    end
end
% K = M21 / sqrt(L1*L2);
time = toc;
